...
async def get_task(request: Request, id: int, user:User = Depends(get_current_user)):
...
async def update_task(request: Request, id: int, data: TaskUpdateInput, user:User = Depends(get_current_user)):
...
async def delete_task(request: Request, id: int, user:User = Depends(get_current_user)):
    task = await Task.filter(id=id, user=user).first()
    if not task:
        return JSONResponse(status_code=404, content={"detail": "Task not found"})
    await task.delete()
